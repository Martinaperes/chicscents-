...
<!-- Message Card -->
<div class="bg-white rounded-xl shadow ring-1 ring-gray-200 p-6 mb-8">
    <div class="flex items-center justify-between mb-6">
        <h3 class="text-lg font-semibold text-gray-900">Message</h3>
        <button type="button" id="help-toggle" class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm bg-blue-50 text-blue-700 hover:bg-blue-100">
    ...
    <div id="help-panel" class="hidden mb-6 rounded-lg border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800">
        ...
        <ul class="list-disc pl-5 space-y-1">
            ...
            <li>Use variables in the answer: <code class="px-1 rounded bg-blue-100">@{{user.name}}</code>, <code class="px-1 rounded bg-blue-100">@{{user.email}}</code>, <code class="px-1 rounded bg-blue-100">@{{session.deal_count}}</code>.</li>
    ...
    <div>
        ...
        <textarea name="question" id="question" 
                  class="w-full rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('question') border-red-500 ring-red-500 @enderror" 
        ...
        @enderror
        <p class="mt-2 text-xs text-gray-500">This question helps identify what scenario this response handles.</p>
    </div>
    ...
    <div>
        ...
        <!-- Tag Input Container -->
        <div class="min-h-[60px] p-3 border-2 border-gray-300 rounded-lg focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-200 @error('keywords') border-red-500 @enderror">
            <div id="keywords-tags" class="flex flex-wrap gap-2 mb-2">
                ...
                @foreach($keywords as $keyword)
                    @if(!empty($keyword))
                        <span class="keyword-tag inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                            {{ trim($keyword) }}
                            <button type="button" onclick="removeKeyword(this)" class="ml-2 text-blue-600 hover:text-blue-800">
        ...
        @error('keywords')
            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
        @enderror
        <p class="mt-2 text-xs text-gray-500">Add multiple keywords. Press Enter or comma to add each keyword.</p>
    </div>

    <!-- Logic Operator -->
    {{-- <div>
        <label for="logic_operator" class="block text-sm font-semibold text-gray-700 mb-2">
            Logic Operator <span class="text-red-500">*</span>
        </label>
        <div class="grid grid-cols-2 gap-4">
            <label class="relative flex items-center p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition {{ old('logic_operator', $botQuestion->logic_operator ?? 'OR') === 'OR' ? 'border-blue-500 bg-blue-50' : 'border-gray-300' }}">
                <input type="radio" name="logic_operator" value="OR" 
                       {{ old('logic_operator', $botQuestion->logic_operator ?? 'OR') === 'OR' ? 'checked' : '' }}
                       class="sr-only" required>
                <div class="flex-1">
                    <div class="font-medium text-gray-900">OR</div>
                    <div class="text-sm text-gray-500">Any keyword matches</div>
                </div>
            ...
            <label class="relative flex items-center p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition {{ old('logic_operator', $botQuestion->logic_operator ?? 'OR') === 'AND' ? 'border-blue-500 bg-blue-50' : 'border-gray-300' }}">
                <input type="radio" name="logic_operator" value="AND" 
                       {{ old('logic_operator', $botQuestion->logic_operator ?? 'OR') === 'AND' ? 'checked' : '' }}
                       class="sr-only" required>
                <div class="flex-1">
                    <div class="font-medium text-gray-900">AND</div>
                    <div class="text-sm text-gray-500">All keywords must match</div>
                </div>
    ...
    <div>
        ...
        <textarea name="answer" id="answer" 
                  class="w-full rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('answer') border-red-500 ring-red-500 @enderror" 
                  rows="6" placeholder="Enter the bot's response message..." required>{{ old('answer', $botQuestion->answer ?? '') }}</textarea>
        @error('answer')
            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
        @enderror
        <div class="mt-2 p-3 bg-blue-50 rounded-lg border border-blue-200">
            <p class="text-xs font-medium text-blue-800 mb-1">Variable Support:</p>
            <p class="text-xs text-blue-700">Use @{{user.name}}, @{{user.email}}, @{{session.deal_count}} for dynamic content.</p>
    </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow ring-1 ring-gray-200 p-6 mb-8">
        ...
        <label class="block text-sm font-semibold text-gray-700 mb-3">Buttons</label>
        <div id="buttons-container" class="space-y-4">
            ...
            @foreach($buttons as $index => $button)
                <div class="button-row p-5 border-2 border-gray-300 rounded-lg bg-gray-50" data-index="{{ $index }}">
                    <div class="flex justify-between items-center mb-4">
                        <span class="text-sm font-medium text-gray-700">Button {{ $index + 1 }}</span>
                        @if($index > 0 || !empty($button['label']) || !empty($button['url']))
                            <button type="button" class="text-red-500 hover:text-red-700 text-sm font-medium remove-button" onclick="removeButton(this)">
                                Remove
                            </button>
                        @endif
                    </div>
        ...
        <button type="button" class="mt-3 flex items-center gap-2 text-blue-600 hover:text-blue-800 text-sm font-medium" onclick="addButton()">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
            </svg>
            Add Button
        </button>
        <p class="mt-2 text-xs text-gray-500">Add interactive buttons to bot responses. URLs support variables.</p>
    </div>

    <!-- Conditions Card -->
    {{-- <div class="bg-white rounded-xl shadow ring-1 ring-gray-200 p-6 mb-8">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">Conditions</h3>
        <label class="block text-sm font-semibold text-gray-700 mb-3">Conditions</label>
        <div id="conditions-container" class="space-y-3">
            ...
            @foreach($conditions as $index => $condition)
                <div class="condition-row p-4 border-2 border-gray-300 rounded-lg bg-gray-50" data-index="{{ $index }}">
                    <div class="flex justify-between items-center mb-3">
                        <span class="text-sm font-medium text-gray-700">Condition {{ $index + 1 }}</span>
                        <button type="button" class="text-red-500 hover:text-red-700 text-sm font-medium" onclick="removeCondition(this)">
                            Remove
                        </button>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-3">
                        <div class="md:col-span-5">
                            <label class="block text-xs font-medium text-gray-600 mb-1">Variable Key</label>
        ...
        @endif
        <button type="button" class="mt-3 flex items-center gap-2 text-blue-600 hover:text-blue-800 text-sm font-medium" onclick="addCondition()">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
            </svg>
            Add Condition
        </button>
        <p class="mt-2 text-xs text-gray-500">Set conditions based on session/user data. Leave empty if no conditions needed.</p>
    </div> --}}
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
            ...
            <input type="number" name="priority" id="priority" 
                   class="w-full rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('priority') border-red-500 ring-red-500 @enderror" 
                   value="{{ old('priority', $botQuestion->priority ?? 0) }}" min="0" required>
            @error('priority')
                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            @enderror
            <p class="mt-2 text-xs text-gray-500">Higher priority questions are matched first. Default: 0</p>
        </div>
        ...
        <div>
            ...
            <label class="flex items-center p-4 border-2 border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 transition">
                ...
                <div class="ml-3">
                    ...
                    <p class="text-xs text-gray-500">Only active questions are matched</p>
                </div>
            </label>
        </div>
    </div>
...
function addKeyword(value) {
    ...
    tag.className = 'keyword-tag inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800';
    tag.innerHTML = `
        ${value}
        <button type="button" onclick="removeKeyword(this)" class="ml-2 text-blue-600 hover:text-blue-800">
            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
    ...
    const html = `
        <div class="button-row p-5 border-2 border-gray-300 rounded-lg bg-gray-50" data-index="${index}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Label</label>
                    <input type="text" name="buttons[${index}][label]" class="w-full rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-500" placeholder="Button Text">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">URL</label>
                    <input type="url" name="buttons[${index}][url]" class="w-full rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-500" placeholder="https://example.com">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Style</label>
                    <select name="buttons[${index}][style]" class="w-full rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-500">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Open In</label>
                    <select name="buttons[${index}][target]" class="w-full rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-500">
    ...
    const html = `
        <div class="condition-row p-4 border-2 border-gray-300 rounded-lg bg-gray-50" data-index="${index}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-3">
                <div class="md:col-span-5">
                    <label class="block text-xs font-medium text-gray-600 mb-1">Variable Key</label>
                    <input type="text" name="conditions[${index}][key]" class="w-full rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-500" placeholder="session.deal_count">
                </div>
                <div class="md:col-span-3">
                    <label class="block text-xs font-medium text-gray-600 mb-1">Operator</label>
                    <select name="conditions[${index}][operator]" class="w-full rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-500">
                        <option value="==">==</option>
                        <option value="!=">!=</option>
                        <option value=">">></option>
                        <option value=">=">>=</option>
                ...
                <div class="md:col-span-4">
                    <label class="block text-xs font-medium text-gray-600 mb-1">Value</label>
                    <input type="text" name="conditions[${index}][value]" class="w-full rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-500" placeholder="Value">
                </div>
            </div>
        </div>
    `;
